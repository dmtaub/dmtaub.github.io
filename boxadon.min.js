...
{let t=[],e=1
for(let s=0;s<10;s++)t.push({levelType:"StarLevel",levelIndexInType:e++})
e=1
for(let s=0;s<5;s++)t.push({levelType:"FloatingStarLevel",levelIndexInType:e++})
...
"triangle"===this.shape?t.fillTriangle(0,this.size.height,this.size.width/2,0,this.size.width,this.size.height):"circle"===this.shape&&t.fillCircle(this.size.width/2,this.size.height/2,this.size.width/2),t.generateTexture(this.textureKey,this.size.width,this.size.height),t.destroy()}}}class d{constructor(){this.items={},this.totalWeight=0}addItem(t,e=1,s=0){this.items[t]?this.items[t].quantity+=e:this.items[t]={quantity:e,weight:s},this.totalWeight+=s*e}removeItem(t,e=1){this.items[t]&&(this.items[t].quantity-=e,this.totalWeight-=this.items[t].weight*e,this.items[t].quantity<=0&&delete this.items[t])}}class p{constructor(t,e={}){if(t instanceof Array==!1)throw new Error("AsciiMap must be an array of strings.")
const s=t.findIndex((t=>t.match(/^-+$/)))
this.asciiMap=s>-1?t.slice(s+1):t,this.levelWidth=this.asciiMap[0].length
for(let t=1;t<this.asciiMap.length;t++)this.asciiMap[t].length>this.levelWidth&&(this.levelWidth=this.asciiMap[t].length)
if(e.repeatCount&&e.repeatCount>0){for(let t=0;t<this.asciiMap.length;t++)this.asciiMap[t]=this.repeatRow(this.asciiMap[t],e.repeatCount)
this.levelWidth*=e.repeatCount}this.tileSize=32,this.levelHeight=this.asciiMap.length,this.spikeTines=e.spikeTines||4}getLevelDimensions(){return{width:this.levelWidth*this.tileSize,height:this.levelHeight*this.tileSize}}repeatRow(t,e){return t.length<this.levelWidth&&(t=t.padEnd(this.levelWidth,".")),t.repeat(Math.ceil(e))}parse(){const t=[],e=[],s=[],i=[],h=[],a={w:(e,s)=>{t.push(this.createWall(e,s))},p:(t,s)=>{e.push(this.createPlatform(t,s))},s:(t,e)=>{s.push(this.createSpike(t,e))},e:(t,e)=>{i.push(this.createEnemy(t,e,"e"))},E:(t,e)=>{i.push(this.createEnemy(t,e,"E"))},h:(t,e)=>{h.push(this.createItem(t,e,"h"))},H:(t,e)=>{h.push(this.createItem(t,e,"H"))}}
for(let t=0;t<this.levelHeight;t++){const e=this.asciiMap[t]
for(let s=0;s<this.levelWidth;s++){const i=a[e[s]]
if(i){i(s*this.tileSize,t*this.tileSize)}}}return{wallData:t,platformData:e,spikeData:s,enemyData:i,itemData:h}}createWall(t,e){return{x:t+this.tileSize/2,y:e+this.tileSize/2,width:this.tileSize,height:this.tileSize,color:9127187}}createPlatform(t,e){return{x:t+this.tileSize/2,y:e+this.tileSize/2,width:this.tileSize,height:this.tileSize/2,color:8388736}}createSpike(t,e){return{x:t+this.tileSize/2,y:e+this.tileSize/2,width:this.tileSize,height:this.tileSize,tines:this.spikeTines,color:8421504}}createEnemy(t,e,s){return{x:t+this.tileSize/2,y:e+this.tileSize/2,type:s}}createItem(t,e,s){return{x:t+this.tileSize/2,y:e+this.tileSize/2,type:s}}}class y{constructor(t,e,s,i){this.scene=t,this.player=s,this.level=i,this.type=e.type,this.x=e.x,this.y=e.y,this.initialX=this.x,this.initialY=this.y,this.resetPatrol=!1,"e"===this.type?(this.color=16753920,this.shape="oval",this.damage=1,this.patrolDistance=200,this.gravity=!0,this.collidesWithLevel=!0,this.maxHealth=2,this.senseDistance=e.senseDistance||150,this.speed=e.speed||50,this.jumpHeight=e.jumpHeight||10,this.jumpSpeed=e.jumpSpeed||-300,this.jumpCooldown=1e3,this.lastJumpTime=0,this.direction=1):"E"===this.type&&(this.color=16711680,this.shape="diamond",this.damage=2,this.patrolDistance=210,this.gravity=!1,this.collidesWithLevel=!1,this.maxHealth=3,this.chaseDistance=e.chaseDistance||300,this.speed=e.speed||100,this.returnSpeed=e.returnSpeed||100,this.tolerance=5,this.state="bobbing",this.direction=1,this.bobAmplitude=20),this.health=this.maxHealth,this.createSprite(),this.collidesWithLevel&&(t.physics.add.collider(this.sprite,i.walls),t.physics.add.collider(this.sprite,i.platforms),t.physics.add.collider(this.sprite,i.spikes)),t.physics.add.overlap(this.sprite,s.sprite,this.hitPlayer,null,this),t.physics.add.overlap(this.sprite,s.projectiles,this.hitByProjectile,null,this),this.sprite.setData("instance",this)}hitByProjectile(t,e){this.health-=1,e.destroy(),this.health<=0?(t.destroy(),this.player.score+=20,this.player.scoreText.setText("Score: "+this.player.score)):this.scene.tweens.add({targets:t,duration:100,alpha:.5,yoyo:!0,repeat:0,onComplete:()=>{t.alpha=1}})}createSprite(){"oval"===this.shape?this.sprite=this.scene.add.ellipse(this.x,this.y,30,40,this.color):"diamond"===this.shape&&(this.sprite=this.scene.add.polygon(this.x,this.y,[0,-20,15,0,0,20,-15,0],this.color)),this.scene.physics.add.existing(this.sprite)}init(){this.sprite.body.setCollideWorldBounds(this.collidesWithLevel),this.sprite.body.onWorldBounds=this.collidesWithLevel,this.sprite.body.setAllowGravity(this.gravity),this.sprite.body.allowGravity=this.gravity}update(t,e){"E"===this.type?this.updateFlyingTypeEnemy(t,e):"e"===this.type&&this.updateWalkingTypeEnemy(t,e)}updateFlyingTypeEnemy(t,e){const i=s.Math.Distance.Between(this.sprite.x,this.sprite.y,this.player.sprite.x,this.player.sprite.y)
switch(this.state){case"bobbing":this.initialY,Math.cos(this.scene.time.now/500),this.bobAmplitude,this.sprite.x>=this.initialX+this.patrolDistance/2?(this.direction=-1,this.sprite.body.setVelocityY(0),this.sprite.y=this.initialY):this.sprite.x<=this.initialX-this.patrolDistance/2&&(this.direction=1,this.sprite.body.setVelocityY(0),this.sprite.y=this.initialY),this.sprite.body.setVelocityX(50*this.direction),i<=this.chaseDistance&&(this.state="chasing")
break
case"chasing":this.scene.physics.moveToObject(this.sprite,this.player.sprite,this.speed),i>this.chaseDistance&&(this.state="returning")
break
case"returning":s.Math.Distance.Between(this.sprite.x,this.sprite.y,this.initialX,this.initialY)>this.tolerance?this.scene.physics.moveTo(this.sprite,this.initialX,this.initialY,this.returnSpeed):(this.sprite.body.setVelocity(0,0),this.direction=1,this.sprite.x=this.initialX,this.sprite.y=this.initialY,this.state="bobbing")}}updateWalkingTypeEnemy(t,e){if(s.Math.Distance.Between(this.sprite.x,this.sprite.y,this.player.sprite.x,this.player.sprite.y)<=this.senseDistance){if(this.resetPatrol=!0,this.sprite.x<this.player.sprite.x?(this.sprite.body.setVelocityX(this.speed),this.direction=1):this.sprite.x>this.player.sprite.x?(this.sprite.body.setVelocityX(-this.speed),this.direction=-1):this.sprite.body.setVelocityX(0),this.jumpHeight>0){const t=this.player.sprite.y-this.sprite.y
Math.abs(this.player.sprite.x-this.sprite.x)<100&&t<-50&&t>-200&&this.scene.time.now-this.lastJumpTime>this.jumpCooldown&&this.sprite.body.onFloor()&&(this.sprite.body.setVelocityY(this.jumpSpeed),this.lastJumpTime=this.scene.time.now)}}else this.resetPatrol&&(s.Math.Within(this.initialX,this.sprite.x,1)||s.Math.Within(this.initialY,this.sprite.y,1))&&(this.resetPatrol=!1,this.initialX=this.sprite.x,this.initialY=this.sprite.y),this.sprite.x>=this.initialX+this.patrolDistance/2?this.direction=-1:this.sprite.x<=this.initialX-this.patrolDistance/2&&(this.direction=1),this.sprite.body.blocked.left?this.direction=1:this.sprite.body.blocked.right&&(this.direction=-1),this.sprite.body.setVelocityX(this.direction*this.speed)}hitPlayer(t,e){this.player.takeDamage(e,t,this.damage)}}class u{constructor(t,e,s){this.scene=t,this.player=s,this.type=e.type,this.x=e.x,this.y=e.y,"h"===this.type?(this.healthAmount=1,this.color=65280,this.size=16):"H"===this.type&&(this.healthAmount=2,this.color=32768,this.size=24),this.createSprite(),t.physics.add.overlap(this.sprite,s.sprite,this.collectItem,null,this),this.sprite.setData("instance",this)}createSprite(){this.sprite=this.scene.add.circle(this.x,this.y,this.size/2,this.color)}collectItem(t,e){this.player.addHealth(this.healthAmount),t.destroy()}}const m=220
class g{constructor(t,e,s={}){this.scene=t,this.kinematics=e,this.levelOptions=s,this.walls=t.physics.add.staticGroup(),this.platforms=t.physics.add.staticGroup(),this.spikes=t.physics.add.staticGroup(),this.stars=t.physics.add.group(),this.enemies=t.physics.add.group(),this.items=t.physics.add.staticGroup(),this.inventory=new d
const i=new Date
this.initLevelData()
const h=new Date
console.log(`Level data init time: ${h-i}ms`),this.createEnemies(),this.createItems(),this.createWalls(),this.createPlatforms(),this.createSpikes()}initLevelData(){if(this.levelOptions.asciiMap){const t=new p(this.levelOptions.asciiMap,{spikeTines:this.levelOptions.spikeTines||1,repeatCount:this.levelOptions.sideScroll||0}),{wallData:e,platformData:s,spikeData:i,enemyData:h,itemData:a}=t.parse(),{width:r,height:o}=t.getLevelDimensions()
this.scene.cameras.main.setBounds(0,-220,r,o+m),this.scene.cameras.main.startFollow(this.scene.player.sprite),this.scene.physics.world.setBounds(0,-220,r,o+m),this.wallData=e,this.platformData=s,this.spikeData=i,this.enemyData=h,this.itemData=a}else this.wallData=[{x:400,y:568,width:800,height:64,color:2263842,name:"ground"},{x:600,y:400,width:150,height:32,color:9127187},{x:50,y:250,width:150,height:32,color:9127187},{x:750,y:220,width:150,height:32,color:9127187}],this.platformData=[{x:400,y:350,width:200,height:16,color:8388736}],this.spikeData=[{x:775,y:522,width:60,height:28,tines:15,color:8421504},{x:25,y:522,width:50,height:28,tines:15,color:8421504}],this.itemData=[],this.enemyData=[]}createEnemies(){this.enemyData.forEach((t=>{const e=new y(this.scene,t,this.scene.player,this)
...
for(let h=0;h<t.tines;h++){let a=t.x-e/2+h*s+s/2+i,r=t.y+t.height/2,o=this.scene.add.triangle(a,r,-s/2,t.height/2,0,-t.height/2,s/2,t.height/2,t.color)
...
i&&i.update(t,e)}))}destroy(){this.walls.clear(!0,!0),this.platforms.clear(!0,!0),this.spikes.clear(!0,!0),this.stars.clear(!0,!0)}}const w="\nw - wall or floor\nP - platform (can jump up through)\ns - spikes (facing up)\nE - flying enemy\ne - walking enemy\nh - health pickup\nH - bigger health pickup\n. - empty space\n---\n........................................\n........................................\n..........E..........e..................\n........................E...............\n.........ppp.........E..................\n......................h.................\n..H..................wwww...............\nwwww....................................\n..................e.....................\n...........h.H..........................\n.........ppppppp........................\n.......................wwwwww...........\n........................................\n........................................\n......................w.w...............\nss....................sss...............\nwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww\nwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww\n".trim().split("\n")
class S extends g{static defaultStarFactor=1
constructor(t,e,s,i,h=1,a){super(t,e,{asciiMap:localStorage.level1?JSON.parse(localStorage.level1).trim().replaceAll("\n\n","\n").split("\n"):w,spikeTines:3,sideScroll:2}),this.difficultySettings=s,this.levelNumber=i,this.levelIndexInType=h
const r=this.difficultySettings.getStarLevelOptions(h)
this.starIncreasePerLevel=r.starIncreasePerLevel,this.starFactor=S.defaultStarFactor+(this.levelIndexInType-1)*this.starIncreasePerLevel,this.createStars(),t.physics.add.collider(this.stars,this.walls,this.starWallCollision,null,this),t.physics.add.collider(this.stars,this.platforms),t.physics.add.collider(this.stars,this.spikes),t.physics.add.collider(this.stars,this.stars),this.scene.physics.add.overlap(this.stars,a.sprite,this.collectStar,null,this),this.player=a}createStars(){let t=this.starFactor
for(let e=0;e<t;e++){let t=s.Math.Between(50,750),e=s.Math.Between(50,300),i=this.scene.add.circle(t,e,7,16776960)
this.scene.physics.add.existing(i),this.stars.add(i),i.body.setBounce(this.kinematics.defaultItemBounce.x,this.kinematics.defaultItemBounce.y),i.body.setCollideWorldBounds(!0),i.body.setVelocity(s.Math.Between(-100,100),s.Math.Between(-100,100))}this.totalStars=t}starWallCollision(t,e){t.body.velocity.x*=.9,t.body.velocity.y*=.9,"ground"==e.name&&(t.body.velocity.x*=.5,t.body.velocity.y*=.5)}collectStar(t,e){e.destroy(),this.player.score+=10,0===this.stars.countActive(!0)&&this.scene.levelComplete()}}class f extends g{constructor(t,e,s,i,h=1,a){super(t,e,s.getFloatingStarLevelOptions(h)),this.difficultySettings=s,this.levelNumber=i,this.levelIndexInType=h,this.numberOfStars=this.levelIndexInType,this.createStars(),this.scene.physics.add.overlap(this.stars,a.sprite,this.collectStar,null,this),this.player=a}createStars(){for(let t=0;t<this.numberOfStars;t++){let t=s.Math.Between(50,750),e=s.Math.Between(50,300),i=this.scene.add.circle(t,e,7,16776960)
...
this.scene.events.on("update",(()=>{a+=r,t.x=e+h*Math.cos(a),t.y=i+h*Math.sin(a)}))}collectStar(t,e){e.destroy(),this.player.score+=10,0===this.stars.countActive(!0)&&this.scene.levelComplete()}}class x extends g{constructor(t,e,s,i,h=1,a){super(t,e),this.player=a,this.difficultySettings=s,this.levelNumber=i,this.levelIndexInType=h
const r=this.difficultySettings.getBallLevelOptions(h)
this.numberOfBalls=r.numberOfBalls,this.elasticity=r.elasticity,this.createBalls(),this.createWinCondition(),t.physics.add.collider(this.stars,this.walls),t.physics.add.collider(this.stars,this.platforms),t.physics.add.collider(this.stars,this.spikes),t.physics.add.collider(this.stars,this.stars),t.physics.add.collider(this.stars,a.sprite)}createBalls(){for(let t=0;t<this.numberOfBalls;t++){let t=s.Math.Between(50,750),e=s.Math.Between(50,300),i=this.scene.add.circle(t,e,10,255)
this.scene.physics.add.existing(i),this.stars.add(i),i.body.setBounce(this.elasticity),i.body.setCollideWorldBounds(!0),i.body.setVelocity(s.Math.Between(-200,200),s.Math.Between(-200,200))}}createWinCondition(){this.winObject=this.scene.add.triangle(400,50,0,60,60,60,30,0,16776960),this.scene.physics.add.existing(this.winObject),this.winObject.body.allowGravity=!1,this.winObject.body.setCollideWorldBounds(!0),this.winObject.body.setImmovable(!0),this.winObject.body.setVelocityX(100),this.winObject.body.setBounce(1,0),this.scene.physics.add.overlap(this.winObject,this.player.sprite,(()=>{this.scene.levelComplete()}))}update(){}}class v{constructor(t,e){this.player=t,this.kinematics=e,this._totalWeight=this.player.inventory.totalWeight,this.baseSpeed=160,this.baseJumpSpeed=this.kinematics.jumpSpeed,this.updateMovementFactors()}get totalWeight(){return this._totalWeight}set totalWeight(t){this._totalWeight!==t&&(this._totalWeight=t,this.updateMovementFactors())}updateMovementFactors(){let t=1-this.totalWeight*this.kinematics.weightSlowFactor
...
this.totalWeight=t}}class b{constructor(t,e,s,i,h){this.scene=t,this.x=e,this.y=s,this.direction=i,this.type=h,this.sprite=t.physics.add.image(this.x,this.y,this.type.textureKey),this.sprite.setScale(1),this.sprite.body.allowGravity=!1,this.sprite.body.setCollideWorldBounds(!1)}init(){let t=this.type.speed
"left"===this.direction?this.sprite.setVelocityX(-t):this.sprite.setVelocityX(t),this.scene.time.delayedCall(1e3,(()=>{this.sprite.destroy()}))}}class I{static defaultMaxHearts=4
constructor(t,e=100,s=450,i={},h,a,r){this.scene=t,this.x=e,this.y=s,this.kinematics=h,this.projectileType=a,this.difficultySettings=r,this.color=void 0===i.playerColor?16711680:i.playerColor,this.maxHearts=i.maxHearts||I.defaultMaxHearts,this.hearts=this.maxHearts,this.recoilTimer=400,this.invulnerable=!1,this.inventory=new d,this.score=0,this.sprite=t.add.rectangle(this.x,this.y,32,48,this.color),t.physics.add.existing(this.sprite),this.sprite.body.setCollideWorldBounds(!0),this.sprite.body.setGravityY(this.kinematics.gravityY),this.facing="right",this.lastFired=0,this.isJumping=!1,this.jumpTime=0,this.movementFactors=new v(this,this.kinematics),this.projectiles=t.physics.add.group(),this.heartsGroup=t.add.group(),this.createHeartsDisplay(),this.scoreText=t.add.text(20,50,"Score: 0",{fontSize:"16px",fill:"#000"}),this.currentUIScrollFactor=0,this.setUIScrollFactor(0)}setUIScrollFactor(t=0){this.currentUIScrollFactor=t,this.scoreText.setScrollFactor(t),this.heartsGroup.children.each((e=>{e.setScrollFactor(t)}))}createHeartsDisplay(){this.heartsGroup.clear(!0,!0)
const t=16711680
let e,s,i
for(e=0;e<this.maxHearts;e++){s=20+20*e,i=20
const h=e<Math.floor(this.hearts)?t:8421504,a=this.scene.add.rectangle(s,20,16,16,h)
if(a.setScrollFactor(this.currentUIScrollFactor),this.heartsGroup.add(a),e<this.hearts){const e=this.scene.add.rectangle(s-4,20,8,16,t)
e.setScrollFactor(this.currentUIScrollFactor),this.heartsGroup.add(e)}}}platformCollisionCallback(t,e){return t.body.velocity.y>0&&!this.scene.inputManager.isDownPressed()}addHealth(t){this.hearts+=t,this.hearts>this.maxHearts&&(this.hearts=this.maxHearts),this.createHeartsDisplay()}update(t,e,s){this.movementFactors.updateTotalWeight()
...
this.flashTween=this.scene.tweens.add({targets:t,duration:this.recoilTimer/e/2-10,alpha:.25,ease:"Linear",yoyo:!0,repeat:e-1,onComplete:()=>{t.alpha=1,this.flashTween=null}})}this.hearts<=0&&this.scene.scene.start("GameOverScene")}}_damageRecoil(t,e,i){const h=new s.Math.Vector2(t.x-e.x,-Math.abs(t.y-e.y)).normalize(),a=i/this.maxHearts*I.defaultMaxHearts*200,r=i/this.maxHearts*I.defaultMaxHearts*100
t.useVelocity?(t.body.touching.left?t.body.velocity.x=a:t.body.touching.right&&(t.body.velocity.x=-a),t.body.velocity.y=-r):(t.body.velocity.x=h.x*a,t.body.velocity.y=h.y*r)}destroy(){this.sprite.destroy(),this.projectiles.clear(!0,!0),this.heartsGroup.clear(!0,!0),this.scoreText.destroy()}}class T extends s.Scene{constructor(){super({key:"MainScene"}),this.debugMode=!1,this.isPaused=!1}init(t){this.currentLevel=t.level||1,this.gameMode=t.gameMode||"easy",this.game.registry.set("gameMode",this.gameMode)}preload(){}create(){this.cameras.main.setBackgroundColor("#87CEEB"),this.difficultySettings=new l,this.kinematics=new n(this.currentLevel),this.projectileType=new c(this,{size:{width:10,height:10},shape:"triangle",speed:500,color:16776960,cooldown:200,recoilForce:100})
let t=this.game.registry.get("playerColor")
if(t=void 0===t?16711680:t,this.player=new I(this,100,450,{maxHearts:4,playerColor:t},this.kinematics,this.projectileType,this.difficultySettings),this.inputManager=new o(this),this.levelProgression=this.difficultySettings.game.getProgression(this.gameMode),this.currentLevel<=this.levelProgression.length){{const t=this.levelProgression[this.currentLevel-1]
let e
switch(t.levelType){case"StarLevel":default:e=S
break
case"FloatingStarLevel":e=f
break
case"BallLevel":e=x}const s=t.levelIndexInType
...
this.add.text(localStorage.width||635,10,`Renderer: ${t}`,{fontSize:"16px",fill:"#eee"}).setScrollFactor(0)}}else this.scene.start("CongratulationsScene",{totalScore:this.player?this.player.score:0})}update(t,e){const i=s.Input.Keyboard.JustDown(this.inputManager.pauseKey),h=s.Input.Keyboard.JustDown(this.inputManager.debugKey)
i&&(this.isPaused=!this.isPaused),h&&(this.debugMode=!this.debugMode,this.isPaused=!0,this.debugMode?(this.cameras.main.setZoom(.5),this.debugCameraX=this.cameras.main.scrollX,this.debugCameraY=this.cameras.main.scrollY,this.player.setUIScrollFactor(1),this.player.scene.cameras.main.stopFollow()):(this.cameras.main.setZoom(1),this.cameras.main.setScroll(this.debugCameraX,this.debugCameraY),this.player.setUIScrollFactor(0),this.player.scene.cameras.main.startFollow(this.player.sprite))),(i||h)&&(this.isPaused?this.physics.world.pause():this.physics.world.resume()),this.debugMode?this.handleDebugCamera(t,e):this.handleGameplay(t,e)}handleGameplay(t,e){this.player.update(this.inputManager,null,t),this.level.update&&this.level.update(t,e)}handleDebugCamera(t,e){const s=e/1e3*1e3,i=this.cameras.main
this.inputManager.isLeftPressed()&&(i.scrollX-=s),this.inputManager.isRightPressed()&&(i.scrollX+=s),this.inputManager.isUpPressed()&&(i.scrollY-=s),this.inputManager.isDownPressed()&&(i.scrollY+=s)}levelComplete(){this.currentLevel+=1,this.game.registry.set("lastLevel",this.currentLevel),this.scene.restart({level:this.currentLevel,gameMode:this.gameMode})}}class k extends s.Scene{constructor(){super({key:"GameOverScene"})}create(){this.selectedOptionIndex=0,this.options=["Title Screen","Retry","Restart"],this.optionButtons=[],this.defaultTextStyle={fontSize:"32px",fill:"#888"},this.highlightedTextStyle={fontSize:"32px",fill:"#fff"},this.cameras.main.setBackgroundColor("#000000"),this.add.text(400,150,"Game Over",{fontSize:"64px",fill:"#a0f"}).setOrigin(.5)
...
break
case 2:this.scene.start("MainScene",{level:1,gameMode:this.game.registry.get("gameMode")})}}shutdown(){this.input.keyboard.shutdown(),this.events.off("shutdown",this.shutdown,this)}}class C extends s.Scene{constructor(){super({key:"CongratulationsScene"})}init(t){this.totalScore=t.totalScore||0}create(){this.cameras.main.setBackgroundColor("#000000"),this.add.text(400,200,"Congratulations!",{fontSize:"64px",fill:"#fff"}).setOrigin(.5),this.add.text(400,300,"Total Score: "+this.totalScore,{fontSize:"32px",fill:"#fff"}).setOrigin(.5),this.add.text(400,400,"Play Again",{fontSize:"32px",fill:"#fff",backgroundColor:"#00ff00"}).setOrigin(.5).setInteractive().on("pointerdown",(()=>{this.scene.start("MainScene",{level:1,gameMode:"easy"})}))}}const D={type:localStorage.useCanvas?s.CANVAS:s.AUTO,scale:{mode:s.Scale.FIT},width:localStorage.width||800,height:localStorage.height||600,physics:{default:"arcade",arcade:{gravity:{y:localStorage.gravity||300},debug:!1}},scene:[i,h,T,k,C]},O=new s.Game(D),M=()=>{setTimeout((()=>{O.scale.setParentSize(window.innerWidth,window.innerHeight)}),100)}
return M(),window.addEventListener("orientationchange",(()=>{M()})),O.scene.start("TitleScene"),O}(Phaser)
