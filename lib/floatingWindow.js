        ...
        ['-', 'X'].forEach((symbol) => {
            ...
            btn.addEventListener('click', () => {
                if (symbol === '-') {
                    ...
                    this.container.style.height = this.container.classList.contains('minimized') ? '30px' : `${startDimensions.height}px`;
                    contentElement.style.display = this.container.classList.contains('minimized') ? 'none' : 'block';
                } else { // 'X'
...
export function createFloatingWindow({
    ...
    const fw = new FloatingWindow(title, contentElement, startDimensions, onTitleChange);
    return {container: fw.container, contentElement: fw.contentElement};
}
