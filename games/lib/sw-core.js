...
let game_bindgen = (function(exports) {
    let script_src;
    if (typeof document !== 'undefined' && document.currentScript !== null) {
        script_src = new URL(document.currentScript.src, location.href).toString();
    }
     ...
     * The SW calls gate.gate_decrypt(game.game_get_session_token(), ct, nonce).
     * @returns {number}
     */
    ...
    /**
     * Generic session-token holder shared by every encrypted game (orca, sunweasel,
     * @param {number} session_token
     * @returns {number}
     */
...
// Loads gate.wasm (auth + decryption) and the per-game wasm (session token holder).
...
// wasm-pack JS glue for gate and the game (with wasm_bindgen renamed to
// gate_bindgen / game_bindgen) before this file, producing the final sw.js
// that is served. That eliminates importScripts entirely. The same sw.js
// serves every game (orca, pewpew, ...) — game identity arrives at unlock.
//
// gate_bindgen and game_bindgen are therefore defined above this point
// in the bundled output.

// Base path of the SW's own directory — '' when served from domain root, '/games/orca' on subpath.
// Per-game assets (pack, wrapped_key, manifest, .enc) are relative to this. The per-dir
// sw.js stub (top-level importScripts '../lib/sw-core.js') keeps swBase == the game dir,
// so all the path logic below is unchanged.
const swBase = self.location.pathname.replace(/\/[^/]+$/, '');

// Shared runtime dir: sibling of the game dir, i.e. <parent>/lib. The gate + generic
// game (session-token) wasm live here once, shared by every game.
// e.g. swBase '/games/orca' → libBase '/games/lib'.
const libBase = swBase.replace(/\/[^/]+$/, '') + '/lib';

// State
let gateModule = null;  // populated after gate_bindgen(`${libBase}/gate_bg.wasm`) resolves
let gameModule = null;  // populated after game_bindgen(`${libBase}/game_bg.wasm`) resolves
...
// Game-agnostic: /_bg\.wasm$/ covers gate_bg.wasm and <game>_bg.wasm; the per-game
// wasm glue (<game>.js) and asset index (<game>/manifest.json) are matched dynamically
// against activeGameName in isPassthrough() below.
const PASSTHROUGH_PATTERNS = [
    /\/sw\.js$/,
    /\/loader\.js$/,
    /\/gate\.js$/, /_bg\.wasm$/,
    /\/wrapped_key\.bin$/,
    // Unencrypted shared library bundles (e.g. pewpew.min.js loaded by the
    // encrypted rail-shooter target). The encrypted game bundle is always
    // fetched as *.min.js.enc (handled by the .enc branch), so a bare *.min.js
    // is never an encrypted asset for any game.
    /\.min\.js$/,
];

function isPassthrough(path) {
    if (PASSTHROUGH_PATTERNS.some((re) => re.test(path))) return true;
    // Per-game wasm glue and asset index (presence signals world-data override).
    if (activeGameName) {
        if (path.endsWith(`/${activeGameName}.js`)) return true;
        if (path.endsWith(`/${activeGameName}/manifest.json`)) return true;
    }
    return false;
}
...
self.addEventListener('message', async (event) => {
    ...
    if (msg.type === 'decrypt_asset') {
        ...
        try {
            ...
            const sessionToken = gameModule.game_get_session_token();
    ...
    try {
        ...
        // gate_bindgen / game_bindgen are defined by the prepended glue in the bundle.
        if (!gateModule) {
            await gate_bindgen(`${libBase}/gate_bg.wasm`);
            gateModule = gate_bindgen;
        }
        if (!gameModule) {
            // Generic session wasm shared by all games (was `${gameName}_bg.wasm`).
            await game_bindgen(`${libBase}/game_bg.wasm`);
            gameModule = game_bindgen;
        }
